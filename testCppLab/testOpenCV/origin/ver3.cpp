...
int main() {
    ...
    // Chuyển markers sang kiểu CV_8U để có thể sử dụng với findContours
    cv::Mat markers8U;
    ...
    for (int i = 0; i < dilatedEdges.rows; i++) {
        for (int j = 0; j < dilatedEdges.cols; j++) {
            if (dilatedEdges.at<uchar>(i, j) == 255) {
                ...
                result.at<cv::Vec3b>(i, j) = cv::Vec3b(0, 0, 255); // Màu đỏ
            }
        }
    }

    // Hiển thị kết quả
    cv::imshow("Vật thể với biên giới nối", result);
    cv::waitKey(0);
